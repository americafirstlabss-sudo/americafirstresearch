    ...
    <article className="group panel overflow-hidden">
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <p className="text-lg font-semibold text-platinum">{formatCurrency(product.price)}</p>
        </div>
        <div className="mt-5 flex items-center gap-3">
          <button type="button" onClick={() => addItem(product)} className="button-primary flex-1">
            Add to Cart
          </button>
          <Link href={href} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-graphite/10 bg-white/85 text-platinum">
